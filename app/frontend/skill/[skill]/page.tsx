...
export default function SkillDetail() {
  ...
  useEffect(() => {
    const fetchSkillData = async () => {
      const response = await fetch("/data/frontend/frontend.json");
  ...
  const chartSeries = [{ name: "提及次数", data: chartData }];
  ...
  return (
    <div className="max-w-screen-sm p-8 mx-auto mt-32 mb-40 bg-white border border-indigo-100 shadow shadow-indigo-100">
      ...
      <hr className="my-4" />
      <div className="mt-8">
        <h2 className="mb-8 text-xl font-bold">📈 每月提及趋势</h2>
        <ul>
      ...
      <div className="mt-8">
        ...
        {complimentarySkills.higherSkills.map((skill, index) => (
          <p key={index} className="mb-4 text-md">
            ...
            <Link href={`/frontend/skill/${skill.skill}`}>
              <span className="inline-block w-32 font-bold text-indigo-500 underline">
                {skill.skill}
              </span>{" "}
              &rarr;{" "}
              <span className="ml-4 font-bold text-indigo-500">

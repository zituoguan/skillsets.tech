  ...
  useEffect(() => {
    const fetchSkillsData = async () => {
      const response = await fetch(
        "/data/technical-writing/technical-writing.json"
      );
    ...
    <>
      <header className="mt-48 text-center animation glow delay-1">
        <h2 className="p-4 text-4xl font-bold sm:p-0">
          The most wanted{" "}
          <span className="font-bold text-indigo-500">Technical Writing</span>{" "}
      ...
      <div className="max-w-screen-sm p-4 mx-auto mt-10 mb-40 animation glow delay-1">
        <div className="grid grid-cols-1 gap-6">
          {filteredAndSortedSkills.map((skill) => {
            return (
              <Link
                href={`/technical-writing/skill/${encodeURIComponent(
                  skill.skill
                )}`}
                key={skill.skill}
              >
